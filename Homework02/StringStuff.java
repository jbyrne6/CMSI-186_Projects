...
public class StringStuff {
   ...
   */
   public static boolean containsVowel( String s ) {
      ...
      String testvar = String.valueOf(teststring.charAt(i));
	  switch(testvar){
		  case "A":
		    return true;
		  case "a":
   ...
   */
   ...
   if (s.length()%2 == 0){
	  for(int i=0; i<(teststring.length()/2); i++){
		  char testvar = teststring.charAt(i);
		  if (testvar != teststring.charAt(teststring.length()-i-1)){
			  return false;
		  }else{
			  continue;
		  }
	   ...
	   for(int i=0; i<(Math.floor(teststring.length()/2)); i++){
		   char testvar = teststring.charAt(i);
		  if (testvar != teststring.charAt(teststring.length()-i-1)){
			  return false;
		  }else{
